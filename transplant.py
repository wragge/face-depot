...
# FACE_CLASSIFIER = '/usr/local/Cellar/opencv/4.3.0_1/share/opencv4/haarcascades/haarcascade_frontalface_default.xml'
...
def get_random_face():
    response = requests.get('https://faceapi.herokuapp.com/faces', params={'n': 1})
...
def add_faces(tweet_image, output_path='output.jpg', max_faces=4):
    ...
    faces = face_cl.detectMultiScale(cv_image, scaleFactor=1.3, minNeighbors=2, minSize=(50, 50), flags=cv.CV_HAAR_SCALE_IMAGE)
    articles = []
    for f_index, (x, y, w, h) in enumerate(faces[:max_faces]):
        # new_faces = list(fd_faces.find({'random_id': {'$near': [random.random(), 0]}}).limit(1))
        new_face_data = get_random_face()
        new_face_url = new_face_data['image_url']
        article_url = new_face_data['article_url']
